...
const content = {
  no: {
    headline: ['Din AI-resepsjonist'],
    subtext: 'Hvert ubesvart anrop er tapt inntekt. Arxon svarer, booker og følger opp — automatisk, døgnet rundt — slik at ingen kunde forsvinner og du kan fokusere på det du gjør best.',
    badgesLabel: 'Integrert med',
    cta1: 'Book gratis konsultasjon',
    cta2: 'Prøv AI-en'
  },
  en: {
    headline: ['Every call.', 'Every customer.', 'Taken care of.'],
    subtext: 'Every missed call is a lost customer. Arxon makes sure nothing falls through the cracks — your AI receptionist answers, books and follows up, so you can focus on what you do best.',
...
export default function LiquidGlassHero({ lang = 'no' }: LiquidGlassHeroProps) {
          ...
          <div className="hero-animate hero-cta flex flex-col sm:flex-row gap-4 md:gap-6 items-center">
            {showHeavy ? (
              <>
                <LiquidMetalButton
                  label={t.cta1}
                  onClick={() => window.open('https://cal.com/arxon/30min', '_blank')}
                />
                <LiquidMetalButton
                  label={t.cta2}
                  viewMode="icon"
                  onClick={() => window.location.href = '#prov-ai'}
                />
              </>
            ) : (
              <>
                <a
                  href="https://cal.com/arxon/30min" target="_blank" rel="noopener noreferrer"
